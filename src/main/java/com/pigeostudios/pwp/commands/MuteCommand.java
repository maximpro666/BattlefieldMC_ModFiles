                ...
                .then(Commands.argument("duration_hours", IntegerArgumentType.integer(1, 720))
                    .then(Commands.argument("reason", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            if (target == punisher) {
                                ctx.getSource().sendFailure(Component.literal("§cНельзя замутить самого себя"));
                                return 0;
                            }
                            ...
                            int id = PunishmentManager.issuePunishment(target.getUUID(),
                                punisherUuid, PunishmentType.MUTE, WarnCategory.CHAT, reason, seconds);

                            ctx.getSource().sendSuccess(() -> Component.literal("§aИгрок " + target.getName().getString() + " заглушен в чате на " + hours + "ч\n§7Причина: " + reason), true);
                            target.sendSystemMessage(Component.literal("§cВы заглушены в чате на " + hours + "ч\n§7Причина: " + reason));
                            return Command.SINGLE_SUCCESS;
                        })))
                .executes(ctx -> {
                    ctx.getSource().sendFailure(Component.literal("§cИспользование: /mute <игрок> <часы> <причина>"));
                    return 0;
                })));
    }
}
