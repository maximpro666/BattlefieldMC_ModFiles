    ...
    public boolean handle(Supplier<NetworkEvent.Context> supplier) {
        ...
        context.enqueueWork(() -> {
            ...
            try {
                ...
                java.nio.file.Path dir = worldDir.resolve("pwp");
                java.nio.file.Files.createDirectories(dir);
                java.nio.file.Files.writeString(dir.resolve("kits.json"), gson.toJson(parsed));

                KitConfig.set(parsed);

                PWP.LOGGER.info("Kit config saved by {} ({} bytes)", player.getName().getString(), configJson.length());
                ...
                PWP.LOGGER.error("Failed to save kit config from {}: {}", player.getName().getString(), e.getMessage());
                player.sendSystemMessage(
