...
import com.yourmod.teamsystem.core.FOBManager;
import com.yourmod.teamsystem.core.FOBManager.SavedFOB;
...
public class FOBCommand {
    ...
    private static int removeFOB(CommandSourceStack source, String name) {
        ...
        boolean removed = TeamSystem.getFOBManager().removeFOBByName(player, name);
        if (!removed) {
    ...
    private static int listFOBs(CommandSourceStack source) {
        ...
        java.util.Optional<com.yourmod.teamsystem.core.Team> teamOpt =
            java.util.Optional.ofNullable(TeamSystem.getTeamManager().getOrCreatePlayerData(player.getUUID()).getTeam());
        if (teamOpt.isEmpty() || !teamOpt.get().isPlayable()) {
            source.sendFailure(Component.literal("§cYou are not on a team"));
            return 0;
        }
        List<SavedFOB> fobs = TeamSystem.getFOBManager().getFOBsForTeam(teamOpt.get());
        if (fobs.isEmpty()) {
